...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new { Emp = e, Dept = d })
            .Join(grades, ed => true, g => true, (ed, g) => new { ed.Emp, ed.Dept, Grade = g })
            .Where(edg => edg.Emp.Sal >= edg.Grade.Losal && edg.Emp.Sal <= edg.Grade.Hisal)
            .Select(edg => new {EName = edg.Emp.EName, DName = edg.Dept.DName, Grade = edg.Grade.Grade
            }).ToList();
